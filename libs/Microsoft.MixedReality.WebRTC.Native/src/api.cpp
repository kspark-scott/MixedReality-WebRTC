...
mrsResult MRS_CALL mrsPeerConnectionCreate(
  ...
  *peerHandleOut = nullptr;
  ...
  if (config.ice_server_urls != nullptr) {
    if (config.num_ice_server_urls > 0) {
      std::vector<std::string> servers =
          SplitString(config.ice_server_urls, ';');
      const size_t num_servers =
          std::min(servers.size(), (size_t)config.num_ice_server_urls);
      rtc_config.servers.reserve(config.num_ice_server_urls);
      webrtc::PeerConnectionInterface::IceServer server;
      for (size_t i = 0; i < num_servers; ++i) {
        const std::string& url = servers[i];
        if (url.length() > 0) {
          server.urls.push_back(url);
        }
      }
      if (config.turn_username) {
        std::string username(config.turn_username);
        if (username.length() > 0) {
          server.username = std::move(username);
        }
      }
      if (config.turn_password) {
        std::string password(config.turn_password);
